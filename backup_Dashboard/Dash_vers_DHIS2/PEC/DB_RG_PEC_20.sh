...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/PEC/DB_RG_PEC_20"

# Define SQL query
SQL_QUERY="SELECT 
    'qacaSxAP7Hu' AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM DATE(rpc.dateEntree)) AS 'period',
    cc.orgunit AS 'orgunit',
    'HllvX50cXC0' AS 'catoptcombo',
    'HllvX50cXC0' AS 'attroptcombo',
    COUNT(rpc.id) AS 'value'
FROM
    register AS r
        LEFT JOIN
    ...
    care_center AS ccr ON rpr.care_center_transferant_id = ccr.id
        LEFT JOIN
    register_pec_controle AS rpc ON rpc.sortie_id = rps.id
WHERE
    pr.id IN (137 , 187)
        AND DATE(rpc.dateEntree) IS NOT NULL
        AND EXTRACT(YEAR_MONTH FROM DATE(rpc.dateEntree)) >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 4 MONTH), '%Y%m')
GROUP BY cc.id , EXTRACT(YEAR_MONTH FROM DATE(rpc.dateEntree))
;"
