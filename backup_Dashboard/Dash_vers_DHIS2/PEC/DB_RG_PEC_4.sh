...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/PEC/DB_RG_PEC_4"

# Define SQL query
SQL_QUERY="SELECT 
    'vnSiS1WCVQM' AS 'dataelement',
...
GROUP BY cc.id , EXTRACT(YEAR_MONTH FROM DATE(rps.date)) "
...
curl -X POST -H "Content-Type: application/csv" -u "${DHIS2_USERNAME}:${DHIS2_PASSWORD}" --data-binary "@${CSV_FILE}_date_$TIMESTAMP.csv" "${DHIS2_URL}" > "${CSV_FILE}_date_$TIMESTAMP_date_$TIMESTAMP.log" 2>&1
