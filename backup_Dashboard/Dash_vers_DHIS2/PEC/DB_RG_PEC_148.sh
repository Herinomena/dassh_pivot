...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/PEC/DB_RG_PEC_148"

# Define SQL query
SQL_QUERY="SELECT 
    'MMfPvH7apjl' AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM DATE(rpe.date)) AS 'period',
    cc.orgunit AS 'orgunit',
    CASE rpst.id
        WHEN 1 THEN 'Y48e2GN7bnp'
        WHEN 5 THEN 'TU6TU8LBF18'
        WHEN 3 THEN 'DpuVtTksJqy'
    END AS 'catoptcombo',
...
WHERE
    pr.id IN (137 , 187)
        ...
        AND rpst.id IS NOT NULL
        AND rps.date is null
        AND EXTRACT(YEAR_MONTH FROM DATE(rpe.date)) >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 4 MONTH), '%Y%m')
GROUP BY cc.id , EXTRACT(YEAR_MONTH FROM DATE(rpe.date)) , rpst.id
;"
