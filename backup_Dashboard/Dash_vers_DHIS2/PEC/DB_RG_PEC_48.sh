...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/PEC/DB_RG_PEC_48"

# Define SQL query
SQL_QUERY="SELECT 
    'rMwZBvyiIUk' AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM DATE(rpe.date)) AS 'period',
    cc.orgunit AS 'orgunit',
    CASE rpmt.id
		when 7 then 'Vf4ROeJvFJI'
		when 19 then 'oYMHQJP30uf'
		when 9 then 'ox44GLDz0aO'
		when 15 then 'yAm9mNutcNc'
		when 17 then 'HJUSqzHkQeh'
		when 1 then 'GZ1t13tJcCU'
		when 11 then 'wHx5TZq7J9Z'
		when 13 then 'tKtIhVkCfyc'
		when 3 then 'AmxbCaLnAVa'
		when 5 then 'Cm5evgX1BFS'
    END AS 'catoptcombo',
