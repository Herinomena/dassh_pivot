...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/PEC/DB_RG_PEC_73"

# Define SQL query
SQL_QUERY="SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT 
    'sXUzXnc26U4' AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM DATE(rpe.date)) AS 'period',
    ccr.orgunit AS 'orgunit',
    'HllvX50cXC0' AS 'catoptcombo',
    'HllvX50cXC0' AS 'attroptcombo',
		round( avg(SUBSTRING_INDEX(timediff(rpia.heure_arrive_centre_referant,rpia.heure_depart_ambulance), ':', 2)),2) AS 'value'
FROM 
    register AS r
        LEFT JOIN
    ...
    register_pec_reference AS rpr ON rpr.entry_id = rpe.id
        LEFT JOIN
    register_pec_info_ambulance AS rpia ON rpia.reference_id = rpr.id
        LEFT JOIN
...
WHERE
    pr.id IN (137,187)
        ...
        and timediff(rpia.heure_arrive_centre_referant,rpia.heure_depart_ambulance)>0
