...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/TB/FormeTB"

# Define SQL query
SQL_QUERY="SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT 
	'ONyt3NBDXtW' AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM rte.date) AS 'period',
  	cc.orgunit AS 'orgunit',
    if(((p.age_unity = 2 AND p.age >= 15 )),
			(CASE rttf.id
                WHEN 1 THEN 'lzk7oIQiFmD'
                WHEN 3 THEN 'bPdN0htVVKF'
                WHEN 5 THEN 'Cpsm4WC3Xhc'
            END),
            if((p.age_unity = 2 AND p.age >= 5 AND p.age <15),
				CASE rttf.id
					WHEN 1 THEN 'JCj5AWgHXzM'
					WHEN 3 THEN 'Vltm46wKKXG'
					WHEN 5 THEN 'xG4D0AJ0qUr'
				END,
            (CASE rttf.id
                WHEN 1 THEN 'UNTLIkuq0Kp'
                WHEN 3 THEN 'LAFwL5qB3rq'
                WHEN 5 THEN 'HSvivvcOKov'
              END))
		)  'catoptcombo',
...
left join register_tuberculosis_controles rtcs on rtcs.register_id=r.id
 ...
 AND EXTRACT(YEAR_MONTH FROM  rte.date) >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 4 MONTH), '%Y%m')
         group by cc.id, EXTRACT(YEAR_MONTH FROM rte.date),rttf.id"
