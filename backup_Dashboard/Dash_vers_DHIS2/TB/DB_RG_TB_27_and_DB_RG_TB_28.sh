...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/TB/DB_RG_TB_27_and_DB_RG_TB_28"

# Define SQL query
SQL_QUERY="SELECT 
   if(v.phase=0 and count(v.id)=8,'z6CFWLTIrEz',if ( v.phase=1 and count(v.id)=4,'XDBAh0QPc9X',null)) AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM vm.date) AS 'period',
...
WHERE
    ...
    vm.date IS NOT NULL
        ...
        AND EXTRACT(YEAR_MONTH FROM  vm.date) >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 4 MONTH), '%Y%m')
GROUP BY cc.id,v.phase,EXTRACT(YEAR_MONTH FROM vm.date)"
