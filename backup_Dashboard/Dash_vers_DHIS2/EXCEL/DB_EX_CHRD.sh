...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/EXCEL/DB_EX_CHRD"

# Define SQL query
SQL_QUERY="SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT
	CASE dbc.name
		WHEN 'DB_EX_CHRD_66' THEN 'IsOCFIBXBFc'
		WHEN 'DB_EX_CHRD_435' THEN 'njQ6XqfZiHR'
	 END AS 'dataelement',
...
WHERE
    dbc.name IN ('DB_EX_CHRD_66','DB_EX_CHRD_435')
