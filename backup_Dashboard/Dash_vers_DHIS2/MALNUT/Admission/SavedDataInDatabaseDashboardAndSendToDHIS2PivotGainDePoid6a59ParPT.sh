...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/MALNUT/Admission/AdmissionCrenasGainDePoid6a59ParPT"

# Define SQL query
SQL_QUERY="SELECT 
    IF(cc.id = 1,
        'YdSxOo99VSQ',
        'ina9CApRla9') AS 'dataelement',
    EXTRACT(YEAR_MONTH FROM rme.date) AS 'period',
  	cc.orgunit AS 'orgunit',
	'HllvX50cXC0' AS 'catoptcombo',
    'HllvX50cXC0' AS 'attroptcombo',
   round(sum(rms.weight - rme.weight),3) AS 'value'
FROM
    register r
        LEFT JOIN
