...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/MALNUT//Sortie/SortieCrenasCreni0a6mois"
...
SELECT 
   if (cc.id= 1,'edCktR1uhwP','DQo3BjJX5kj') AS 'dataelement',
    ...
    case rmem.id
	when 3 then 'WLVKUaliJUk'
	when 133 then 'mClEEuIUnw8' 
	when 5 then 'BynSKSXC40Z'   
    when 1 then 'HUkKuzzleO5'   
	when 11 then 'fiH143GgmCf'
    when 9 then 'Oabmk9HYKT2'
    when 7 then 'XMNE1M7bHe3'
    END
     AS 'catoptcombo',
...
FROM
 ...
 ((p.age_unity = 0 and p.age >= 0 AND p.age <= 31) or (p.age_unity = 1 and p.age >= 1 AND p.age < 6))
