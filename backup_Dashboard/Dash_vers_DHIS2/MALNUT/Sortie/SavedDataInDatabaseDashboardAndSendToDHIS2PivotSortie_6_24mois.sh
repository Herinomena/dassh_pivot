...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/MALNUT/Sortie/SortieCrenasCreni6a24mois"
...
SELECT 
   if (cc.id= 1,'edCktR1uhwP','DQo3BjJX5kj') AS 'dataelement',
    ...
    case rmem.id
	when 3 then 'Sq9zU1ZUbT0'
	when 133 then 'OvV7vkp1F2A' 
	when 5 then 'PicgtHARTzL'   
    when 1 then 't9G20biHuuX'   
	when 11 then 'LX63W8TnBBF'
    when 9 then 'YhTPaUQGrb4'
    when 7 then 'C1U0ccCIOA8'
    END
     AS 'catoptcombo',
...
FROM
  ...
  ((p.age_unity = 1 AND p.age >= 6 AND p.age <= 24) OR (p.age_unity = 2 AND p.age >= 1 AND p.age <= 2))
