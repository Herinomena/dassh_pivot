...
CSV_FILE="/home/herinomena/Documents/Export_dashbord_DHIS/Script_Migration_Dash_vers_DHIS2/MALNUT/Sortie/SortieCrenasCreni2a5ans"
...
SELECT 
   if (cc.id= 1,'edCktR1uhwP','DQo3BjJX5kj') AS 'dataelement',
    ...
    case rmem.id
	when 3 then 'tNMHEwYQ1tc'
	when 133 then 'wE5GIbZccuk' 
	when 5 then 'gxINqPsm2F8'   
    when 1 then 'neb4DOXg44N'   
	when 11 then 'IRtL4lBjg9K'
    when 9 then 'dCH7GFSN4QZ'
    when 7 then 'a5nWbmeqqES'
    END
     AS 'catoptcombo',
...
FROM
   ...
   ((p.age_unity = 1 AND p.age > 24 AND p.age <= 59) OR (p.age_unity = 2 AND p.age >2 AND p.age < 5))
